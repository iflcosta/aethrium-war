...
local function creatureSayCallback(cid, type, msg)
	...
	elseif msgcontains(msg, "balance") then
		...
		elseif player:getBankBalance() >= 100000 then
			...
			npcHandler:say("Your account balance is " .. player:getBankBalance() ..
				               " gold.", cid)
			return true
		end
	elseif msgcontains(msg, "deposit") then
		...
		if msgcontains(msg, "all") then
			count[cid] = player:getMoney()
			npcHandler:say("Would you really like to deposit " .. count[cid] .. " gold?",
			               cid)
			...
			if string.match(msg, "%d+") then
				...
				npcHandler:say(
					"Would you really like to deposit " .. count[cid] .. " gold?", cid)
				npcHandler.topic[cid] = topicList.DEPOSIT_CONSENT
				return true
			else
				npcHandler:say(
					"Please tell me how much gold it is you would like to deposit.", cid)
	...
	elseif npcHandler.topic[cid] == topicList.DEPOSIT_CONSENT then
		if msgcontains(msg, "yes") then
			if player:getMoney() >= tonumber(count[cid]) then
				...
				npcHandler:say("Alright, we have added the amount of " .. count[cid] ..
	...
	elseif msgcontains(msg, "withdraw") then
		if string.match(msg, "%d+") then
			count[cid] = getMoneyCount(msg)
			if isValidMoney(count[cid]) then
				npcHandler:say("Are you sure you wish to withdraw " .. count[cid] ..
					               " gold from your bank account?", cid)
		...
		if msgcontains(msg, "yes") then
			if player:canCarryMoney(count[cid]) then
				if not player:withdrawMoney(count[cid]) then
					...
					npcHandler:say("Here you are, " .. count[cid] ..
	...
	elseif msgcontains(msg, "transfer") then
		...
		if #parts < 3 then
			if #parts == 2 then
				...
				if isValidMoney(count[cid]) then
					npcHandler:say("Who would you like transfer " .. count[cid] .. " gold to?",
					               cid)
				...
				npcHandler:say(
					"Please tell me the amount of gold you would like to transfer.", cid)
			...
			local seed = 3
			if #parts > 3 then seed = parts[3] == "to" and 4 or 3 end
			for i = seed, #parts do receiver = receiver .. " " .. parts[i] end
			...
			if isValidMoney(count[cid]) then
				...
				if player:getName() == transfer[cid].name then
					npcHandler:say(
						"Why would you want to transfer money to yourself? You already have it!",
						cid)
					npcHandler.topic[cid] = topicList.NONE
					return true
				end
				...
				if transfer[cid] then
					if transfer[cid].vocation == VOCATION_NONE and Player(cid):getVocation() ~=
						0 then
						npcHandler:say(
							"I'm afraid this character only holds a junior account at our bank. Do not worry, though. Once he has chosen his vocation, his account will be upgraded.",
							cid)
						npcHandler.topic[cid] = topicList.NONE
						return true
					end
					npcHandler:say("So you would like to transfer " .. count[cid] ..
						               " gold to " .. transfer[cid].name .. "?", cid)
	...
	elseif npcHandler.topic[cid] == topicList.TRANSFER_PLAYER_WHO then
		...
		if transfer[cid] then
			...
			npcHandler:say(
				"So you would like to transfer " .. count[cid] .. " gold to " ..
					transfer[cid].name .. "?", cid)
			npcHandler.topic[cid] = topicList.TRANSFER_PLAYER_CONSENT
		else
			npcHandler:say("This player does not exist.", cid)
			npcHandler.topic[cid] = topicList.NONE
	...
	elseif npcHandler.topic[cid] == topicList.TRANSFER_PLAYER_CONSENT then
		if msgcontains(msg, "yes") then
			if not player:transferMoneyTo(transfer[cid], count[cid]) then
				...
				npcHandler:say("Very well. You have transfered " .. count[cid] ..
					               " gold to " .. transfer[cid].name .. ".", cid)
				transfer[cid] = nil
			end
		elseif msgcontains(msg, "no") then
	...
	elseif msgcontains(msg, "change") then
		...
		if #parts > 1 then
			...
			elseif parts[2]:lower() == "platinum" then
				npcHandler:say(
					"Would you like to change your platinum coins into {gold} or {crystal}?",
					cid)
				npcHandler.topic[cid] = topicList.CHANGE_PLATINUM_GOLD_OR_CRYSTAL
			elseif parts[2]:lower() == "crystal" then
				npcHandler:say(
					"How many crystal coins would you like to change into platinum?", cid)
				npcHandler.topic[cid] = topicList.CHANGE_CRYSTAL_PLATINUM
			end
		else
			npcHandler:say(
				"Would you like to change {gold}, {platinum} or {crystal} coins?", cid)
			npcHandler.topic[cid] = topicList.CHANGE_GOLD_CHOOSE
		end
	elseif msgcontains(msg, "gold") and npcHandler.topic[cid] ==
		topicList.CHANGE_GOLD_CHOOSE then
		npcHandler:say("How many platinum coins would you like to get?", cid)
		npcHandler.topic[cid] = topicList.CHANGE_GOLD_PLATINUM
	elseif msgcontains(msg, "platinum") and npcHandler.topic[cid] ==
		topicList.CHANGE_GOLD_CHOOSE then
		npcHandler:say(
			"Would you like to change your platinum coins into gold or crystal?", cid)
		npcHandler.topic[cid] = topicList.CHANGE_PLATINUM_GOLD_OR_CRYSTAL
	elseif msgcontains(msg, "crystal") and npcHandler.topic[cid] ==
		topicList.CHANGE_GOLD_CHOOSE then
		npcHandler:say(
			"How many crystal coins would you like to change into platinum?", cid)
		npcHandler.topic[cid] = topicList.CHANGE_CRYSTAL_PLATINUM
	elseif npcHandler.topic[cid] == topicList.CHANGE_GOLD_PLATINUM then
		if getMoneyCount(msg) < 1 then
			...
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] * 100 ..
				               " of your gold coins into " .. count[cid] ..
				               " platinum coins?", cid)
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_PLATINUM_GOLD_OR_CRYSTAL then
		if msgcontains(msg, "gold") then
			npcHandler:say("How many platinum coins would you like to change into gold?",
			               cid)
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_PLATINUM_GOLD then
		if getMoneyCount(msg) < 1 then
			...
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] ..
				               " of your platinum coins into " .. count[cid] * 100 ..
				               " gold coins for you?", cid)
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_PLATINUM_CRYSTAL then
		if getMoneyCount(msg) < 1 then
			...
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] * 100 ..
				               " of your platinum coins into " .. count[cid] ..
				               " crystal coins for you?", cid)
	...
	elseif npcHandler.topic[cid] == topicList.CHANGE_CRYSTAL_PLATINUM then
		if getMoneyCount(msg) < 1 then
			...
			count[cid] = getMoneyCount(msg)
			npcHandler:say("So you would like me to change " .. count[cid] ..
				               " of your crystal coins into " .. count[cid] * 100 ..
				               " platinum coins for you?", cid)
...
npcHandler:setMessage(MESSAGE_GREET,
                      "Welcome to the bank, |PLAYERNAME|! Need some help with your {bank account}?")

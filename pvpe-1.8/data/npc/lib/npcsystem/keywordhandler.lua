...
if not KeywordHandler then
	...
	function KeywordNode:getParent() return self.parent end

	-- Returns an array of the callback function parameters assosiated with this node.
	function KeywordNode:getParameters() return self.parameters end

	-- Returns an array of the triggering keywords assosiated with this node.
	function KeywordNode:getKeywords() return self.keywords end
	...
	function KeywordNode:addAliasKeyword(keywords)
		...
		local new = KeywordNode:new(keywords, prevNode.callback, prevNode.parameters,
		                            prevNode.condition, prevNode.action)
	...
	KeywordHandler = {root = nil, lastNode = nil}
	...
	function KeywordHandler:reset(cid)
		if self.lastNode[cid] then self.lastNode[cid] = nil end
	end
	...
	function KeywordHandler:processNodeMessage(node, cid, message)
		...
		for i, childNode in pairs(node.children) do
			if childNode:checkMessage(messageLower) then
				...
				if childNode:processMessage(cid, message) then return true end
	...
	function KeywordHandler:getRoot() return self.root end
	...
	function KeywordHandler:moveUp(cid, steps)
		if not steps or type(steps) ~= "number" then steps = 1 end

		for i = 1, steps do
			if not self.lastNode[cid] then return nil end

...
void Player::updateInventoryWeigth()
{
	...
	if(access < g_config.ACCESS_HOUSE) {
...
bool Player::addItem(Item *item, bool test /*=false*/){
	...
	if(access < g_config.ACCESS_HOUSE && getFreeCapacity() < item->getWeight()) {
		return false;
	}
...
void Player::setVocation(playervoc_t voc)
{
	if (vocation == VOCATION_NONE)
	{
		if (voc == VOCATION_DRUID || voc == VOCATION_KNIGHT || voc == VOCATION_PALADIN || voc == VOCATION_SORCERER)
			...
			std::cout << "Player: setVocation: invalid vocation!" << std::endl;
	}
	else
		std::cout << "Player: setVocation: already has vocation!" << std::endl;
}
#endif //YUR_ROOKGARD
...
#ifdef TDS_BOX
void Player::sendBox(Creature *c)
{
     client->sendBox(c);
}
#endif //TDS_BOX

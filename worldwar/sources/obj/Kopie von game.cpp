...
void GameState::onAttack(Creature* attacker, const Position& pos, const MagicEffectClass* me)
{
	...
	for(cit = tile->creatures.begin(); cit != tile->creatures.end(); ++cit) {
		...
		if (damage > 0) {
            ...
            if(attackPlayer && targetPlayer && attackPlayer->guildName == targetPlayer->guildName && attackPlayer->guildStatus >= GUILD_MEMBER && targetPlayer->guildStatus >= GUILD_MEMBER)
                damage = 0;
...
{
if (playerMoving && (*brn)->getID() == 1492 && player != thing || playerMoving && (*brn)->getID() == 1493 && player != thing || playerMoving && (*brn)->getID() == 1494 && player != thing || playerMoving && (*brn)->getID() == 1491 && player != thing || playerMoving && (*brn)->getID() == 1496 && player != thing || playerMoving && (*brn)->getID() == 2123 && player != thing || playerMoving && (*brn)->getID() == 2124 && player != thing || playerMoving && (*brn)->getID() == 2125 && player != thing)
{
player->sendCancel("Sorry, not possible.");
return;
}

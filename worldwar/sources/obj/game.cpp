	...
	for(cit = tile->creatures.begin(); cit != tile->creatures.end(); ++cit) {
		...
		if (damage > 0) {
			...
			pvpArena = isPvpArena(attacker) && isPvpArena(targetCreature);
                ...
                if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 63 && targetPlayer->lookfeet == 63){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
                if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 94 && targetPlayer->lookfeet == 94){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
                if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 50 && targetPlayer->lookfeet == 50){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
                if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 125 && targetPlayer->lookfeet == 125){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
			    if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 114 && targetPlayer->lookfeet == 114){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
				}
				if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 57 && targetPlayer->lookfeet == 57){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
			    if(attackPlayer && targetPlayer && me->offensive && attackPlayer->lookfeet == 118 && targetPlayer->lookfeet == 118){
				attackPlayer->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
				damage = 0;
			}
		...
		if (me->offensive && game->getWorldType() == WORLD_TYPE_PVP_ENFORCED)
...
void GameState::onAttack(Creature* attacker, const Position& pos, Creature* attackedCreature)
{
    ...
    if(attackedPlayer && attacker)
        attackedPlayer->sendBox(attacker);
	...
	const Position& toPos, const Container* toContainer, const Item* toItem, int count)
{
	...
	if(!fromItem->isPickupable()) {
		...
		if((!fromContainer || !player->isHoldingContainer(fromContainer)) && player->isHoldingContainer(toContainer)) {
			if(player->access < g_config.ACCESS_HOUSE && player->getFreeCapacity() < itemWeight) {
	...
	slots_t toSlot, const Item *toItem, int count)
{
	...
	double itemWeight = (fromItem->isStackable() ? Item::items[fromItem->getID()].weight * std::max(1, count) : fromItem->getWeight());
	if(player->access < g_config.ACCESS_HOUSE && !player->isHoldingContainer(fromContainer) &&
...
//code here, text should be the name
if(hasWildcard(text)) {
std::string name = getNameByWildcard(text);
if(name != "") {
//
}
					...
					for(CreatureStateVec::const_iterator csIt = creatureStateVec.begin(); csIt != creatureStateVec.end(); ++csIt) {
						...
						if(spectator->CanSee(target->pos.x, target->pos.y, target->pos.z))
						{
							...
							if(creatureState.manaDamage > 0){
								...
								spectator->sendAnimatedText(target->pos, 9, manaDmg.str());
							}
		...
		if (attackedCreature->manaShieldTicks < 1000 && (creatureState.damage == 0) &&
			...
			if (spectator->CanSee(attackedCreature->pos.x, attackedCreature->pos.y, attackedCreature->pos.z))
			{
				...
				if(creatureState.manaDamage >0) {
					...
					spectator->sendAnimatedText(attackedCreature->pos, 9, manaDmg.str());
				}
...
void Game::playerSetAttackedCreature(Player* player, unsigned long creatureid)
{
	...
	if(!attackedCreature || (attackedCreature->lookfeet == 94 && player->lookfeet == 94 || (attackedCreature->lookfeet == 50 && player->lookfeet == 50 || (attackedCreature->lookfeet == 125 && player->lookfeet == 125 || (attackedCreature->lookfeet == 57 && player->lookfeet == 57 || (attackedCreature->lookfeet == 114 && player->lookfeet == 114 || (attackedCreature->lookfeet == 118 && player->lookfeet == 118 || (attackedCreature->lookfeet == 63 && player->lookfeet == 63 || (attackedCreature->access >= g_config.ACCESS_PROTECT || ((getWorldType() == WORLD_TYPE_NO_PVP || rook) &&
		!pvpArena && player->access < g_config.ACCESS_PROTECT && (dynamic_cast<Player*>(attackedCreature) || attackedIsSummon))))))))))) {
	if(attackedCreature) {
		  player->sendTextMessage(MSG_SMALLINFO, "You may not attack your teammates.");
    }
		player->sendCancelAttacking();
		...
		player->setAttackedCreature(attackedCreature);  // starting decaying
		stopEvent(player->eventCheckAttacking);
		player->eventCheckAttacking = addEvent(makeTask(500, std::bind2nd(std::mem_fun(&Game::checkCreatureAttacking), player->getID())));
	}
...
//ambiguous
std::cout << "EU QUERO DROGASSSSSSSSS!!!" << std::endl;
return;
}
else {
//name doesn't exist
player->sendMagicEffect(player->pos, NM_ME_PUFF);
player->sendTextMessage(MSG_SMALLINFO, "A player with this name does not exist.");
return;
}
}
...
c = getCreatureByName(name);
		...
		if (dynamic_cast<Player*>(c))
		{
			...
			{
				player->sendMagicEffect(player->pos, NM_ME_PUFF);
				player->sendTextMessage(MSG_SMALLINFO, "You do not have enough mana.");
				return;
			}
...
void Game::onPvP(Creature* creature, Creature* attacked, bool murder)
{
	...
	{
      std::string ownedtext(std::string("Sorry! You got pwned by ") + player->getName() + ".");
      attackedPlayer->sendTextMessage(MSG_RED_INFO, ownedtext.c_str());

		player->inFightTicks = g_config.WHITE_TIME;

		if (attackedPlayer->skullType == SKULL_RED)
		{
			player->skullKills++;
			std::string justice(std::string("Congratulations! You pwned ") + attackedPlayer->getName() + ".");
	...
	if (player)
	{
		std::ostringstream info;
		    info << "You got now " << player->skullKills 
			<< " pwned player(s).";
...
void Game::LeaveParty(Player *player)
{
	...
	bericht1 << player->getName() << " has left the team";
	...
	player->sendTextMessage(MSG_INFO, "You have left the team.");
	player->party = 0;
}
...
void Game::disbandParty(unsigned long partyID)
{
	...
	for(AutoList<Player>::listiterator cit = Player::listPlayer.list.begin(); cit != Player::listPlayer.list.end(); ++cit)
	{
		if((*cit).second->party == partyID)
		{
			...
			(*cit).second->sendTextMessage(MSG_INFO, "Your team has been disbanded.");
		}
	}
}
#endif //TLM_SKULLS_PARTY
...
void Game::checkShutdown(int minutes)
{
	...
	else
	{
		...
		msg << "Server is restarting in " << minutes << (minutes>1? " minutes" : " minute") << std::ends;
...
if(name.length() > wantedname.length()) {//without this check, it was crashing my server now and then

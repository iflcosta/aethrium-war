...
if(line[1] == "city")
depotid = 1;
else if(line[1] == "lakeshire") // add as much as you want
depotid = 2;
else
canSend = false;
...
long oldmana = player->getMana();
    if(game->creatureSaySpell(player, text)){
        type = SPEAK_SAY;
    if(oldmana == player->getMana())
    {
	...
	if(creature){
		...
		if(add_player == player){
			...
			*/
			...
			GetMapDescription(player->pos.x-8, player->pos.y-6, player->pos.z, 18, 14, msg);
#endif //TRS_GM_INVISIBLE
			AddMagicEffect(msg,player->pos, NM_ME_MORT_AREA);
			...
			*/
			...
			// login bubble
			AddMagicEffect(msg,creature->pos, NM_ME_MORT_AREA);
		}
	}
...
void Protocol76::parseInviteParty(NetworkMessage &msg)
{
	...
	if (target->party != 0)
	{
		...
		bericht << target->getName() << " is already in a team.";
	...
	if(player->getSex() == PLAYERSEX_MALE){
		bericht2 << player->getName() <<" invites you to his team.";
	}else{
		bericht2 << player->getName() <<" invites you to her team.";}
...
void Protocol76::parseJoinParty(NetworkMessage &msg)
{
	...
	std::stringstream bericht1;
	bericht1 <<  "You have joined "<< target->getName() << "'s team.";
	player->sendTextMessage(MSG_INFO, bericht1.str().c_str());

	std::stringstream bericht2;
	bericht2 << player->getName() << " has joined the team.";
...
void Protocol76::parsePassLeadership(NetworkMessage &msg)
{
	...
	target->sendTextMessage(MSG_INFO, "You are now leader of your team.");
	std::stringstream bericht;
	bericht << target->getName() << " is now leader of your team.";
...
#ifdef TDS_BOX
void Protocol76::sendBox(Creature *c)
{
     if(!c)return;
     NetworkMessage msg;
     msg.AddByte(0x86);
     msg.AddU32(c->getID());
     msg.AddByte(0x00);//black
     WriteBuffer(msg);
}
#endif //TDS_BOX

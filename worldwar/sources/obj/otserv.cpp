...
#include "mdump.h"
MiniDumper miniDumper("YurOTS Danger II Edition");
		...
		if (protId == 0x0201)
		{
			...
			if(isclientBanished(s)) {
				...
				if (account.accnumber == accnumber && passwordTest(password,account.password)) // seems to be a successful load
				{
					...
					{
						msg.AddString((*it));
						if(g_game.getPlayerByName((*it)) != NULL)
						msg.AddString("Online");
						else
						msg.AddString("Offline");
						msg.AddU32(serverip);
				...
				if(IOAccount::instance()->getPassword(accnumber, name, acc_pass) && passwordTest(password,acc_pass))
				{
					...
					if(s){
						...
						else if (player->banned)
						{
							...
							msg.Reset();
							msg.AddByte(0x14);
							msg.AddString("Server closed for 1 minutes.");
							msg.WriteToSocket(s);
						}
#ifdef YUR_LOGIN_QUEUE
...
int main(int argc, char *argv[])
{
	...
	std::cout << ":: YurOTS 0.9.4d STF Edition" << std::endl;
	std::cout << ":: ~~~~~~By Danger II~~~~~~~" << std::endl;
	std::cout << ":: ~~~~~~~~~~~~~~~~~~~~~~~~~" << std::endl;
...
//	LOG_MESSAGE("main", EVENT, 1, "Starting server");
	...
	// load monster data
	std::cout << ":: Loadding monsters.xml...         ";
